# Repo Audit - Portfolio Evidence Pass

Audited on 2026-07-04 from local clones plus `gh repo view` metadata for `smgpulse007`.

| Repo | Status | Public-safe artifacts / implementation signals | Portfolio use | Risks or gaps |
| --- | --- | --- | --- | --- |
| `smgpulse007.github.io` | Pages portfolio | Astro, Tailwind, React islands, GitHub Pages workflow, case-study assets | Primary portfolio site | Keep all routes and Pages deploy healthy |
| `smgpulse007` | Profile README gateway | Short README, SVG banner | GitHub profile entry point | Should remain concise and point to the site |
| `hospital-readmission-fhir-ml-api` | Flagship | FastAPI app, Dockerfile, compose file, `pyproject.toml`, synthetic FHIR bundles, sample response | Lead healthcare case study | Synthetic demo only; no clinical deployment claim |
| `hl7-ai-challenge` | Flagship | Docker Compose service graph, HL7/FHIR services, RabbitMQ/Postgres/Redis, submission README, architecture PNGs | Healthcare interoperability case study | Challenge/demo framing only |
| `llm-steering` | Flagship | FastAPI + React workbench, docs assets, activation steering diagrams/GIFs, research notes | Agentic/LLMOps and interpretability case study | Research lab; no model-safety guarantee |
| `AlphaQuant` | Flagship | Docker stack, FastAPI, worker/API Dockerfiles, UI screenshots/GIFs, sample reports | Agentic market-intelligence case study and homepage quant spotlight | Engineering demo; not investment advice |
...
| `R_Projects` | Not surfaced | R scripts and public/sample datasets | Not surfaced | Older coursework/project files; avoid overloading portfolio |

Flagship conclusion: the Pages site can safely feature Readmission FHIR ML API, HL7 AI Challenge, LLM Steering, AlphaQuant, FreshTrack, Local Document AI, ChatWithWiki, and NFL Forecasting when each is framed with maturity labels and responsible-use boundaries. Healthcare and insurance remain the lead brand; forecasting and quant are breadth evidence, not advice products.

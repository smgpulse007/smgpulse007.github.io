...
GitHub Actions portability follow-up: the initial staging-review heads exposed narrow homepage and page-hero overflow only on Ubuntu fallback-font metrics (18–19px at 320px initially, then 4px at 360px and 16px at 320px after the first correction). A fluid 320–600px type scale, sized against the longest unbroken heading word, added real layout margin. The functional width gate now waits for `document.fonts.ready`, matching the visual harness, and collects every offending element before failing. After synchronization isolated a settled 34px `/experience/` overflow at 320px on hosted Firefox/WebKit, the lower Experience grids received explicit shrink boundaries, responsive era/evidence headings, and scoped emergency word wrapping. The resulting all-route diagnostics then proved that hosted Firefox/WebKit were including the closed mobile disclosure's unstyled contents in document width, so the drawer now has an explicit hidden state independent of user-agent `<details>` styling. A runner artifact then isolated intrinsic sizing on the remaining one-column mobile grids; those tracks now use `minmax(0, 1fr)`, direct children have explicit shrink bounds, and the intentional Lab filter scroller has a bounded width. Once the boxes were bounded, the runner measurements exposed the final source directly: wider Linux fallback-serif glyphs inside long words. The affected About, Recognition, Contact, and résumé headings now use word-length-aware fluid mobile sizes with scoped emergency wrapping. Final correction proof includes a 72/72 all-route 320/360px screenshot pass, 18/18 affected-surface screenshots, 6/6 post-hardening Experience screenshots, 36/36 final word-fit screenshots at 320/360/390px, 96/96 E2E cases in CI-mode, a 9/9 repeated cross-engine width stress run, a clean uninterrupted repository validation, and a serial 96/96 Ubuntu Noble Playwright 1.61.1 container run. The hosted PR check remains a mandatory release gate.
